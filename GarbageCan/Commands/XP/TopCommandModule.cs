    ...
    public class TopCommandModule : BaseCommandModule
    {
        ...
        public Task TopCommand(CommandContext ctx)
        {
            Task.Run(async () =>
            {
                ...
                {
                    var member = await ctx.Guild.GetMemberAsync(users[i].id);
                    builder.AppendFormat("{0} - {1} (lvl {2}, {3} xp)", i + 1, member.DisplayName, users[i].lvl, users[i].xp);
                    builder.AppendLine();
                }
                ...
                builder.AppendLine("--");
                builder.AppendFormat("{0} - {1} (lvl {2}, {3} xp)", senderIndex + 1, ctx.Member.DisplayName, users[senderIndex].lvl, users[senderIndex].xp);
